...
const allowreplay = false;
const allowreauth = false;
...
function resizeScreen() {
  console.log({ cols: term.cols, rows: term.rows }, 'Resize the window');
  fitAddon.fit();
  websocket.send(
    new TextEncoder().encode(`\x01${JSON.stringify({ cols: term.cols, rows: term.rows })}`)
  );
...
// safe shutdown
// const hasCountdownStarted = false;
